...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const accountFilter = {
      $and: [
        { 'wordPowerAnalysis.overallScore': { $exists: true, $ne: null } },
        {
          $or: [
    ...
    // Get videos with word power analysis for this corporate account only
    const allVideos = await videoAnalysis.find(accountFilter)
      .sort({ 'uploadInfo.uploadDate': 1 })
      .toArray();
    
    console.log(`Found ${allVideos.length} videos with word power analysis`);
    ...
    // Fixed baseline targets for word power improvement
    const baselineScore = 42; // Target baseline for word power
    ...
    for (const userId in userVideos) {
      ...
      if (videos.length >= 2) {
        ...
        const firstScore = videos[0].wordPowerAnalysis?.overallScore || 0;
        const latestScore = videos[videos.length - 1].wordPowerAnalysis?.overallScore || 0;
        ...
        const score = videos[0].wordPowerAnalysis?.overallScore || 0;
    ...
    const totalScoreAllVideos = allVideos.reduce((sum, video) => 
      sum + (video.wordPowerAnalysis?.overallScore || 0), 0
    );
    ...
    console.log('Word Power Overall Improvement:', result);
    return NextResponse.json(result);
    
  } catch (error) {
    console.error('Error fetching word power improvement data:', error);
    return NextResponse.json(
      { error: 'Failed to fetch word power improvement data' },
      { status: 500 }
    );
  } finally {
    if (client) {
